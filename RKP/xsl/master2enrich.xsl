  ...
  <teiHeader>
    ...
    <xsl:if test="page">
  	<xsl:element name="facsimile">
  	  <xsl:variable name="carovy_kod">
  	    <xsl:value-of select="substring(/page[1]/pgImage[1]/@href, 38, 10)"/>
  	  </xsl:variable>
  	  <xsl:variable name="file_name">
  	    <xsl:value-of select="concat(/msDescription/msIdentifier/idno, '.log.xml')"/>
  	  </xsl:variable>
  	  <xsl:variable name="sysno">
  	    
  	    <xsl:value-of select="document('{$file_name}')/manuscript/sysno"/>
  	  </xsl:variable>
  		<xsl:variable name="http">
  		  <xsl:value-of select="concat('http://hades.mzk.cz/media/mzk03/', substring($sysno, 1, 3), '/',
  		    substring($sysno, 4, 3), '/', substring($sysno, 6, 3), '/', $carovy_kod)"/>
