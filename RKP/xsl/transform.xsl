...
<xsl:template match="msHeading">
    ...
    <xsl:if test="respStmt"><tr><th>Impresum</th><td><xsl:value-of select="respStmt"/></td></tr></xsl:if>
   <xsl:if test="origDate"><tr><th>Datace</th><td><xsl:value-of select="origDate"/><xsl:text> </xsl:text><xsl:choose><xsl:when test="origDate/@notAfter and origDate/@notBefore">(<xsl:value-of select="origDate/@notBefore"/>-<xsl:value-of select="origDate/@notAfter"/>)</xsl:when><xsl:otherwise><xsl:if test="origDate/@notAfter">(ante quam <xsl:value-of select="origDate/@notAfter"/>)</xsl:if> <xsl:if test="origDate/@notBefore">(post quam <xsl:value-of select="origDate/@notBefore"/>)</xsl:if></xsl:otherwise></xsl:choose> </td></tr></xsl:if>
...
<xsl:template match="msIdentifier">
    ...
    <table><xsl:if test="country"><tr><th>Země</th><td><xsl:value-of select="country"/></td></tr></xsl:if>
   <xsl:if test="region"> <tr><th>Oblast</th><td><xsl:value-of select="region"/></td></tr></xsl:if>
   <xsl:if test="settlement"><tr><th>Místo</th><td><xsl:value-of select="settlement"/></td></tr></xsl:if>
...
<xsl:template match="msContents">
    ...
    <xsl:apply-templates select="overview"/> 
     <xsl:if test="//msContents/msItem"><h6>Části rukopisu</h6></xsl:if>
 ...
 <xsl:if test="respStmt/resp"><tr><th>Údaje o odpovědnosti</th> <td><xsl:value-of select="respStmt/resp"/><xsl:text> </xsl:text><xsl:if test="respStmt/resp//date[@value]">[<xsl:value-of select="respStmt/resp//date/@value"/>]</xsl:if></td></tr></xsl:if>
 <xsl:if test="note"><tr><th>Poznámka</th> <td><xsl:value-of select="note"/></td></tr></xsl:if>
 <xsl:if test="bibl[1]"><tr><th>Literatura</th> <td><xsl:value-of select="bibl[1]"/> </td></tr></xsl:if>
 <xsl:if test="bibl[2]"><tr><th>Literatura</th> <td><xsl:value-of select="bibl[2]"/> </td></tr></xsl:if>
 <xsl:if test="bibl[3]"><tr><th>Literatura</th> <td><xsl:value-of select="bibl[3]"/> </td></tr></xsl:if>
 <xsl:if test="bibl[4]"><tr><th>Literatura</th> <td><xsl:value-of select="bibl[4]"/> </td></tr></xsl:if>
 <xsl:if test="listBibl/bibl[1]"><tr><th>Literatura</th> <td><xsl:value-of select="listBibl/bibl[1]"/> </td></tr></xsl:if>
 <xsl:if test="listBibl/bibl[2]"><tr><th>Literatura</th> <td><xsl:value-of select="listBibl/bibl[2]"/> </td></tr></xsl:if>
 <xsl:if test="listBibl/bibl[3]"><tr><th>Literatura</th> <td><xsl:value-of select="listBibl/bibl[3]"/> </td></tr></xsl:if>
 <xsl:if test="listBibl/bibl[4]"><tr><th>Literatura</th> <td><xsl:value-of select="listBibl/bibl[4]"/> </td></tr></xsl:if>
   </table>
    <xsl:if test="//msContents/msItem/msItem">
  	<xsl:for-each select="msItem">
       <h5>Textová jednotka (n: <xsl:value-of select="@n"/>)</h5>
   <table>
  <xsl:if test="locus"><tr><th>Rozsah</th> <td><xsl:value-of select="locus"/><xsl:text> </xsl:text> <xsl:choose><xsl:when test="locus/@from and locus/@to">(from: <xsl:value-of select="locus/@from"/><xsl:text> </xsl:text>to: <xsl:value-of select="locus/@to"/>)</xsl:when><xsl:otherwise><xsl:if test="locus/@to">(to:  <xsl:value-of select="locus/@to"/>)</xsl:if>
  <xsl:if test="locus/@from">(from:  <xsl:value-of select="locus/@from"/>)</xsl:if></xsl:otherwise></xsl:choose>
  </td></tr></xsl:if>
  <xsl:if test="author"><tr><th>Autor</th> <td><xsl:value-of select="author"/></td></tr></xsl:if>
...
<xsl:template match="physDesc">
    ...
    <xsl:if test="support/watermarks"><tr><th>Filigrán</th><td><xsl:value-of select="support/watermarks"/></td></tr></xsl:if>
