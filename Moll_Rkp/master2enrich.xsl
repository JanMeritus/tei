...
Version 0.81-2009-02-24
  ...
  <xsl:output doctype-system="enrich.dtd" indent="yes" method="xml"/>
  ...
  <!-- Přemysl Bar, 2010-10-12 -->
  <xsl:template match="date[not(@value)]">
    <xsl:choose>
      <xsl:when test="contains(substring(., 2, 1), '.') and contains(substring(., 4, 1), '.')">
        <xsl:element name="date">
          <xsl:attribute name="when">
            <xsl:value-of select="substring(., 5, 4)"/>-0<xsl:value-of select="substring(., 3,
              1)"/>-0<xsl:value-of select="substring(., 1, 1)"/>
          </xsl:attribute>
          <xsl:value-of select="."/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="contains(substring(., 2, 1), '.') and contains(substring(., 5, 1), '.')">
        <xsl:element name="date">
          <xsl:attribute name="when">
            <xsl:value-of select="substring(., 6, 4)"/>-<xsl:value-of select="substring(., 3,
              2)"/>-0<xsl:value-of select="substring(., 1, 1)"/>
          </xsl:attribute>
          <xsl:value-of select="."/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="contains(substring(., 3, 1), '.') and contains(substring(., 5, 1), '.')">
        <xsl:element name="date">
          <xsl:attribute name="when">
            <xsl:value-of select="substring(., 6, 4)"/>-0<xsl:value-of select="substring(., 4,
              1)"/>-<xsl:value-of select="substring(., 1, 2)"/>
          </xsl:attribute>
          <xsl:value-of select="."/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="contains(substring(., 3, 1), '.') and contains(substring(., 6, 1), '.')">
        <xsl:element name="date">
          <xsl:attribute name="when">
            <xsl:value-of select="substring(., 7, 4)"/>-<xsl:value-of select="substring(., 4,
              2)"/>-<xsl:value-of select="substring(., 1, 2)"/>
          </xsl:attribute>
          <xsl:value-of select="."/>
        </xsl:element>
      </xsl:when>
  ...
  <!-- Vanish text element -->
  <xsl:template match="manuscript/text">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <!-- q deleted in ENRICH  <xsl:template match="q">
    
    <quote>
      <xsl:apply-templates select="@*|*|comment()|processing-instruction()|text()"/>
    </quote> 
  </xsl:template>
  Přemysl Bar - 25.8.2010 -->
        ...
        <xsl:attribute name="xml:id">
          <xsl:choose>
            <xsl:when test="msIdentifier/idno">
              <xsl:value-of
                select="concat('CZ_BOA001_',
                document('seznam_rkp2.xml')/seznam/item/carovy_kod[preceding-sibling::signatura[1] = $idno])"/>
            </xsl:when>
            <xsl:otherwise>
  	  ...
  	  <xsl:if test="altName">
  	    <altIdentifier>
  	      <xsl:element name="idno">
  	        <xsl:attribute name="type">
  	          <xsl:value-of select="concat(substring-before(altName/@type, ' '), '_',
  	            substring-after(altName/@type, ' '))"/>
  	        </xsl:attribute>
  	        <xsl:value-of select="altName"/>
  	      </xsl:element>
  	     
  	      <note>Signatura podle Vladislava Dokoupila</note>
  	    </altIdentifier>
  	  </xsl:if>
  	</msIdentifier>
  </xsl:template>
  ...
  <xsl:template match="physDesc">
    <physDesc>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test="form/p">
            <p><term><xsl:apply-templates select="form/p/child::node()"/></term></p>
              ...
              <xsl:if test=" support |extent | collation |foliation |condition">
                <supportDesc>
                  <xsl:choose>
                    ...
                    <xsl:when test="contains(translate(support, $uc, $lc), 'papír')">
                      <xsl:attribute name="material">chart</xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
  ...
  <xsl:template match="support">
    <xsl:element name="{name()}">
      <xsl:choose>
        ...
        <xsl:if test="overview/p">
          <xsl:apply-templates select="overview/p/node()"/>
        </xsl:if>
      <xsl:if test="watermarks">
  ...
  <xsl:template match="watermarks">
    <xsl:choose>
      ...
      <watermark><xsl:apply-templates/></watermark>
      </xsl:for-each>
      </xsl:when>
      <xsl:otherwise>
        <watermark><xsl:apply-templates/></watermark>
  ...
  <xsl:template match="msWriting/handDesc/p">
    <xsl:apply-templates select="child::node()[normalize-space(.)]"/>
  </xsl:template>
  <xsl:template match="msWriting[handDesc]/p">
    <xsl:choose>
      <xsl:when test="bibl or q">
        <summary><note><xsl:apply-templates/></note></summary>
      </xsl:when>
      <xsl:otherwise>
...
-->
  ...
  <xsl:template match="handDesc">
    ...
    <!--   <xsl:if test="@medium">
      ...
      <xsl:choose>
        <xsl:when
          test="@script= 'carolmin' or @script= 'textualis' or @script= 'cursiva' or @script=
          'hybrida' or @script= 'humbook' or script='humcursiva' or @script='other' or @script">
        ...
        <xsl:when test="contains(translate(@script, $uc, $lc), 'bastar')">
          <xsl:attribute name="script">bastarda</xsl:attribute>
        </xsl:when>
        <xsl:when
      ...
      <xsl:choose><xsl:when test="@lang">
        <xsl:attribute name="xml:lang">
          <xsl:value-of select="translate(@lang, $uc, $lc)"/>
        </xsl:attribute>
      </xsl:when></xsl:choose>
      <xsl:if test="p">
        <xsl:for-each select="p">
  ...
  <!-- Přemysl Bar - 24.8.2010 -->
  <xsl:template match="handDesc/p/locus">
    
    <xsl:choose>
      <xsl:when test="contains(@from, 'r') or contains(@from, 'v')">
        <xsl:text>na f. </xsl:text>
        <locus><xsl:value-of select="@from"/></locus><xsl:text>-</xsl:text><locus><xsl:value-of select="@to"/></locus>
      </xsl:when>
      
      <xsl:when test="contains(., 'r') or contains(., 'v')">
        <locus><xsl:value-of select="."/></locus>
      </xsl:when>
      <xsl:when test="child::text() and not(contains(., 'r')) and not(contains(., 'v'))">
        <locus><xsl:value-of select="."/></locus>
      </xsl:when>
      <xsl:otherwise>
        <xsl:text>na p. </xsl:text>
        <locus><xsl:value-of select="@from"/></locus><xsl:text>-</xsl:text><locus><xsl:value-of select="@to"/></locus>
      </xsl:otherwise>
  ...
  <xsl:template match="msItem/q">
   <!-- change msItem/q to quote  <quote>
      <xsl:apply-templates/>
    </quote>
    -->
    <!--
   ...
   <q><xsl:apply-templates select="child::node()"/></q>
  </xsl:template>

  <!-- Přemysl Bar, 22.9.2010 -->
  <xsl:template match="msItem/locus">
  	<xsl:choose>
  		<xsl:when test="contains(@from, 'r') or contains(@from, 'v') or contains(@to, 'r') or contains(@to, 'v')">
  		...
  		<xsl:when test="not(@from) and not(@to)">
  			<locus><xsl:value-of select="child::node()" /></locus>
  		</xsl:when>
  		
  			<xsl:when test="not(contains(@from, 'r')) or not(contains(@from, 'v')) or not(contains(@to, 'r')) or not(contains(@to, 'v'))">
  				<xsl:element name="locus">
  				<xsl:attribute name="from"><xsl:value-of select="@from" /></xsl:attribute>
  				<xsl:attribute name="to"><xsl:value-of select="@to" /></xsl:attribute>
  				<xsl:text>p. </xsl:text>
...
-->
 ...
 Přemysl Bar, 22.9.2010-->
  ...
  <xsl:template name="textLang">
    <xsl:value-of select="concat(translate(substring(//msHeading/textLang,1,1),$lc,$uc),
      substring(//msHeading/textLang, 2))" />
</xsl:template>
...
<!-- Přemysl Bar, 22.09.2010 -->
<xsl:template match="msContents">
	<xsl:choose>
		<xsl:when test="not(overview)">
		<msContents>	
		<xsl:element name="textLang">
				<xsl:attribute name="mainLang">
				  <xsl:apply-templates
				  select="//msHeading/textLang/@langKey"/>
				  <xsl:if test="//msHeading/textLang/@otherLangs">
				    <xsl:text> </xsl:text>
				  <xsl:apply-templates
				    select="//msHeading/textLang/@otherLangs"/> 
				  </xsl:if>
				  <xsl:if test="//msHeading/textLang/@lang">
				  <xsl:text> </xsl:text> 
				  <xsl:apply-templates
				      select="//msHeading/textLang/@lang"/>
				  </xsl:if>
				</xsl:attribute>
...
<xsl:template match="msItem/textLang">
	<xsl:element name="textLang">
	  <xsl:attribute name="mainLang"><xsl:apply-templates select="@langKey"/> <xsl:apply-templates
	    select="@otherLangs"/>  <xsl:apply-templates select="@lang"/> </xsl:attribute>
				<xsl:value-of select="concat(translate(substring(.,1,1),$lc,$uc), substring(.,2))" />
			</xsl:element>
</xsl:template>
  ...
  <xsl:template match="dimensions[height and width]">
    <dimensions>
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:attribute name="type">binding</xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="@*[not(name()='type')]|*"/>
    </dimensions>
...
-->
  ...
  <xsl:template match="name">
    ...
    <xsl:choose>
      <xsl:when test="contains(., 'Moravská zemská knihovna') or contains(., 'knihovna') or
        contains(., 'spol')">
    ...
    <supplied>
      ...
      <xsl:choose>
        ...
        <xsl:when
          ...
          ><xsl:attribute name="reason">omitted</xsl:attribute></xsl:when>
          <xsl:when test="contains(translate(@reason, $uc, $lc), 'illeg')"><xsl:attribute
            name="reason">illegible</xsl:attribute></xsl:when>
          <xsl:when test="contains(translate(@reason, $uc, $lc), 'dam')"><xsl:attribute
            name="reason">damage</xsl:attribute></xsl:when>
          <xsl:when
            test="contains(translate(@reason, $uc, $lc), 'unk') or contains(translate(@reason, $uc, $lc), 'oth')"
            ><xsl:attribute
              name="reason">unknown</xsl:attribute></xsl:when>
  ...
  <teiHeader>
        <fileDesc>
            ...
            <xsl:apply-templates select="msDescription"/>
  	
  	 <listPerson>
  	   <xsl:for-each select="//msHeading/author">
  	     <person>
  	       <persName><xsl:value-of select="//msHeading/author"/></persName>
  	       
  	     </person>
  	   </xsl:for-each>
  	   <xsl:for-each select="//msItem/respStmt/resp">
  	     <person>
  	       <persName><xsl:value-of select="name/child::text()"/></persName>
  	       <xsl:if test="name/date">
  	         <floruit><xsl:value-of select="name/date"/></floruit>
  	       </xsl:if>
  	       <xsl:if test="child::text()">
  	         <occupation><xsl:value-of select="child::text()"/></occupation>
  	       </xsl:if>
  	     </person>
  	   </xsl:for-each>
  	 </listPerson>
    ...
    <revisionDesc>
      <xsl:if test="//additional/adminInfo/recordHist/change">
      <xsl:element name="change">
        <xsl:attribute name="when">
          <xsl:value-of select="//additional/adminInfo/recordHist/change/item/p/date/@value"/>
        </xsl:attribute>
        <xsl:element name="persName">
        <xsl:value-of
            select="//additional/adminInfo/recordHist/change/respStmt/name[@type='person' and
            position()=last()]"/>
      </xsl:element>
      </xsl:element>
      </xsl:if>
     <xsl:element name="change">
        <xsl:attribute name="when">
          <xsl:text>2010-12-10</xsl:text>
        </xsl:attribute>
        <xsl:element name="persName">
          <xsl:text>Přemysl Bar</xsl:text>
        </xsl:element>
       <xsl:element name="note">
         <xsl:text>Konverze z MASTER do TEI</xsl:text>
       </xsl:element>
      </xsl:element>
    </revisionDesc>
    ...
    <xsl:if test="page">
  	<xsl:element name="facsimile">
  	  ...
  	  <xsl:variable name="file_name">
  	    <xsl:value-of select="concat(//msDescription/msIdentifier/idno/text(), '.tei.xml')"/>
  	  </xsl:variable>
  	  <xsl:variable name="http">
  	    
  	    <xsl:value-of select="document('$file_name')//TEI/facsimile/@xml:base"/>
  	  </xsl:variable>
  		...
  		<xsl:for-each select="page">
  		  <!--<xsl:variable name="obrazek">
  		    <xsl:value-of select="substring(pgImage/@href, 77, 5)"/>
  		  </xsl:variable>-->
  		<xsl:element name="surface">
  		  <xsl:choose>
  		    <xsl:when test="pgImage/@id">
  		      <xsl:attribute name="xml:id">
  		        
  		        <xsl:text>id_</xsl:text>
  		        <!--   <xsl:text>adl_10_</xsl:text>  -->
  		    <!--     <xsl:choose>
  		          <xsl:when test="pgPagination/text()">
  		            <xsl:value-of select="pgPagination"/>
  		          </xsl:when>
  		          <xsl:when test="pgFoliation/text()">
  		            <xsl:value-of select="pgFoliation"/>
  		          </xsl:when>
  		        </xsl:choose>
  		    --> 		         
  		        <!-- <xsl:choose>
  		           <xsl:when test="$obrazek >= 00042">
  		             <xsl:value-of select="concat(substring-after(pgImage/@id, 'ID'), 'P')"/>
  		           </xsl:when>
  		           <xsl:otherwise>
  		             <xsl:value-of select="substring-after(pgImage/@id, 'ID')"/>  
  		           </xsl:otherwise>
  		         </xsl:choose>
  		        -->
  		        <xsl:value-of select="substring-after(pgImage/@id, 'ID')"/> 
  		    ...
  		    <xsl:otherwise>
  		      <xsl:attribute name="xml:id">
  		        <xsl:text>id_</xsl:text><xsl:number count="page" format="00001"/>
  		      </xsl:attribute>
  		    </xsl:otherwise>
  		  </xsl:choose>
  			...
  			<xsl:element name="desc">
  				...
  				<xsl:element name="label">
  				  <!--  <xsl:text>Adl.7 </xsl:text> -->
  				  <xsl:choose>
  				    <xsl:when test="pgDescription/pgText/pgItem[text()]">
  				  
              				  <xsl:choose>
              				    <xsl:when test="contains(substring(pgDescription/pgText/pgItem, 1, 3), '000')">
              				      <xsl:value-of select="normalize-space(substring(pgDescription/pgText/pgItem, 4))" />
              				    </xsl:when>
              				    <xsl:when test="substring(pgDescription/pgText/pgItem, 1, 2) =
              				      '00'">
              				      <xsl:value-of select="normalize-space(substring(pgDescription/pgText/pgItem, 3))" />
              				    </xsl:when>
              				    <xsl:when test="substring(pgDescription/pgText/pgItem, 1, 1) = '0'">
              				      <xsl:value-of select="normalize-space(substring(pgDescription/pgText/pgItem, 2))" />
              				    </xsl:when>
              				    <xsl:otherwise>
              				      <xsl:value-of select="pgDescription/pgText/pgItem"/>
              				    </xsl:otherwise>
  				    ...
  				    <xsl:otherwise>
  				      <xsl:choose>
  				        <xsl:when test="pgPagination">
  				         
  				      <!--<xsl:number count="page" format="I"/>-->
                  				      <xsl:choose>
                  				        
                  				        <xsl:when test="contains(substring(pgPagination, 1, 3), '000')">
                  				          <xsl:value-of select="normalize-space(substring(pgPagination, 4))" />
                  				        </xsl:when>
                  				        <xsl:when test="substring(pgPagination, 1, 2) =
                  				          '00'">
                  				          <xsl:value-of select="normalize-space(substring(pgPagination, 3))" />
                  				        </xsl:when>
                  				        <xsl:when test="substring(pgPagination, 1, 1) = '0'">
                  				          <xsl:value-of select="normalize-space(substring(pgPagination, 2))" />
                  				        </xsl:when>
                  				        <xsl:otherwise>
                  				          <xsl:value-of select="pgPagination"/>
                  				        </xsl:otherwise>
  				        ...
  				        <xsl:when test="pgFoliation">
  				          <xsl:value-of select="pgFoliation"/>
                  				        <!--<xsl:choose>
                  				          
                  				          <xsl:when test="contains(substring(pgFoliation, 1, 3), '000')">
                  				            <xsl:value-of select="normalize-space(substring(pgFoliation, 4))" />
                  				          </xsl:when>
                  				          <xsl:when test="substring(pgFoliation, 1, 2) =
                  				            '00'">
                  				            <xsl:value-of select="normalize-space(substring(pgFoliation, 3))" />
                  				          </xsl:when>
                  				          <xsl:when test="substring(pgFoliation, 1, 1) = '0'">
                  				            <xsl:value-of select="normalize-space(substring(pgFoliation, 2))" />
                  				          </xsl:when>
                  				          <xsl:otherwise>
                  				            <xsl:value-of select="pgFoliation"/>
                  				          </xsl:otherwise>
  			...
  			<xsl:apply-templates select="pgImage"/>
  		 <!--  <xsl:for-each select="pgImage[4]">
  		    <xsl:variable name="url">
  		      <xsl:value-of select="@href" />
  		    </xsl:variable>
          		  <xsl:element name="graphic">
          		    <xsl:attribute name="url">
          		      <xsl:value-of select="concat('/BW', substring($url, 57, 19), '.JPG')" />
          		    </xsl:attribute>
          		  </xsl:element>
    ...
    <!--   <facsimile><graphic></graphic></facsimile> -->    
  	...
  	<xsl:variable name="url">
  		<xsl:value-of select="@href" />
  				...
  				<xsl:element name="graphic">
  					<xsl:attribute name="url">
  					  <xsl:choose>
  					    <xsl:when test="contains($url, 'priloha')">
  					      <xsl:value-of select="substring($url, 56)" />
  					    </xsl:when>
  					    <xsl:otherwise>
  					      <xsl:value-of select="substring($url, 49)" />
  					    </xsl:otherwise>
  					  </xsl:choose>
  ...
  <xsl:template match="recordHist">
    <source>
      <xsl:apply-templates select="source"/>
      <xsl:apply-templates select="//additional/adminInfo/recordHist/change/date"/>
       <xsl:apply-templates select="//additional/adminInfo/recordHist/change/respStmt/name"/>
      <xsl:apply-templates select="//additional/adminInfo/recordHist/change/item/p[child::node()]"/>
    </source>
  </xsl:template>
  
  <xsl:template match="source">
    <xsl:choose>
      <xsl:when test="p/@lang">
        <xsl:for-each select="p">
        <xsl:element name="p">
          <xsl:attribute name="xml:lang">
  ...
  <xsl:template match="bibl/ref">
   ...
   <xsl:choose> 
     <xsl:when test="parent::node()[contains(., 'Knihopis')]">
  ...
  <xsl:template match="layout">
    <layout>   
      <xsl:choose>
        <xsl:when test="contains(p, 'dvou sloupcích')">
  ...
  <xsl:template match="origDate/@evidence">
  ...
  <xsl:template match="signatures">
    <bibl><idno><xsl:value-of select="."/></idno></bibl>
  </xsl:template>
  ...
  <xsl:template match="decoration[contains(decoNote/@type, 'notation')]">
    <musicNotation><xsl:apply-templates select="decoNote//*"/></musicNotation>
  </xsl:template>
