        ...
        <xsl:attribute name="xml:id">
          <xsl:choose>
            <xsl:when test="msIdentifier/idno">
              <xsl:value-of
                select="concat('CZ_BOA001_', document('seznam.xml')/seznam/item[idno =
                $idno]/carovy_kod)"/>
            </xsl:when>
            <xsl:otherwise>
  ...
  <xsl:template match="msIdentifier">
    <xsl:variable name="signatura">
  	...
  	<msIdentifier>
  	  ...
  	  <idno><xsl:value-of select="document('seznam.xml')/seznam/item[idno = $signatura]/sig"/></idno>
  	  <altIdentifier>
  	    <idno><xsl:value-of select="idno"/></idno>
  	    <note>Signatura v soupisu Vladislava Dokoupila, viz <bibl>Dokoupil, Vladislav: Soupis rukopisů
  	      mikulovské ditrichsteinské knihovny. Praha 1958</bibl></note>
  	  </altIdentifier>
  	  ...
  	  <xsl:if test="altName">
  	    <altIdentifier>
  	      ...
  	      <note>Stará signatura původní sbírky</note>
  	    </altIdentifier>
  	  </xsl:if>
  	</msIdentifier>
  </xsl:template>
...
<xsl:template match="msItem/textLang">
	<xsl:element name="textLang">
	  ...
	  <xsl:if test="@lang='CZE' or @lanKey='CZE'">
...
-->
  ...
  <xsl:template match="profileDesc">
    <revisionDesc>
      <xsl:if test="//additional/adminInfo/recordHist/change/item/p/date">
      <xsl:element name="change">
        <xsl:attribute name="when">
          <xsl:if test="//additional/adminInfo/recordHist/change/item/p/date/@value">
            <xsl:value-of select="//additional/adminInfo/recordHist/change/item/p/date/@value"/>
          </xsl:if>
