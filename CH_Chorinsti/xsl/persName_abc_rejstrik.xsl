...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <jmena>
            <xsl:apply-templates select="jmena/item[1]"/>
            <xsl:apply-templates
                select="/jmena/item[name/child::node() !=
                preceding-sibling::item[1]/name/child::node()]"
            />
        </jmena>
    </xsl:template>
    <xsl:template match="item">
        <xsl:variable name="name" select="name"/>
        <xsl:variable name="pocet" select="count(following::name[. = $name])"/>
        <item>
            <name>
