...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="sourceDesc">
        <xsl:apply-templates
            select="//placeName[not(parent::persName) and not(parent::msIdentifier) and
            not(parent::altIdentifier) and not(parent::recordHist/source/p)]">
            <xsl:sort lang="cze"/>
        </xsl:apply-templates>
    </xsl:template>
    <xsl:template match="placeName">
        ...
        <item>
            ...
            <signatura><xsl:value-of select="preceding::msIdentifier/idno"/></signatura>
        </item>
    </xsl:template>
