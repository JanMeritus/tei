        ...
        <xsl:attribute name="xml:id">
          <xsl:choose>
            <xsl:when test="msIdentifier/idno">
              <xsl:value-of
                select="concat('CZ_BOA001_', document('seznam_rkp.xml')/seznam/item/carovy_kod[preceding-sibling::signatura =
                $idno])"/>
            </xsl:when>
            <xsl:otherwise>
...
-->
  ...
  <xsl:template match="handDesc/p/locus">
  	...
  	<xsl:choose>
  		...
  		<xsl:when test="@from and @to and not(contains(@from, 'r')) and not(contains(@from, 'v'))">
  			<xsl:text>na p. </xsl:text>
  			<locus><xsl:value-of select="@from"/></locus><xsl:text>-</xsl:text><locus><xsl:value-of select="@to"/></locus>
  		</xsl:when>
  		<xsl:when test="contains(., 'r') or contains(., 'v')">
  			<xsl:text>na f. </xsl:text><locus><xsl:value-of select="."/></locus>
  		</xsl:when>
  		<xsl:when test="not(contains(., 'r')) and not(contains(., 'v'))">
  			<xsl:text>na p. </xsl:text><locus><xsl:value-of select="."/></locus>
  		</xsl:when>
  		<xsl:otherwise/>
...
-->
 ...
 Přemysl Bar, 22.9.2010-->
  ...
  <xsl:template match="textLang" name="textLang">
    <xsl:value-of select="concat(translate(substring(.,1,1),$lc,$uc),
      substring(., 2))" />
</xsl:template>
...
<xsl:template match="msContents">
	<xsl:choose>
		...
		<msContents>	
		  <xsl:for-each select="//msHeading/textLang">
		<xsl:element name="textLang">
		  ...
		  <titlePage>
		    <docTitle><xsl:value-of select="//msHeading/title"/></docTitle>
		    <docImprint><pubPlace><xsl:value-of
		      select="//msHeading/respStmt/resp/name[@type='place'][@role='printer']"/></pubPlace><xsl:text>
		      </xsl:text>
		      <publisher><xsl:value-of
		        select="//msHeading/respStmt/resp/name[@type='person'][@role='printer']"/></publisher><xsl:text>
		      </xsl:text> <xsl:value-of
		          select="//msHeading/origDate"/></docImprint>
		  </titlePage>
			<xsl:apply-templates/>
		</msContents>	
		</xsl:when>
...
<xsl:template match="msItem/textLang">
	<xsl:element name="textLang">
				<xsl:attribute name="mainLang"><xsl:apply-templates select="@langKey"/> <xsl:apply-templates select="@otherLangs"/>   </xsl:attribute>
    ...
    <xsl:if test="page">
  	<xsl:element name="facsimile">
  		<xsl:variable name="http">
  			<xsl:value-of select="page[1]/pgImage[2]/@href" />
  		</xsl:variable>
  		
  		
  			<xsl:attribute name="xml:base">
  				<xsl:value-of select="substring-before($http, 'low')" />
  			</xsl:attribute>
  		...
  		<xsl:element name="surface">
  			<xsl:attribute name="xml:id">
  				<xsl:text>id</xsl:text><xsl:number count="page" />
  			</xsl:attribute>
  			<xsl:element name="desc">
